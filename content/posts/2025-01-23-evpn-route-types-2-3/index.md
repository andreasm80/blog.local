...
In the previous section I covered how reachability information was exhanged between VTEPS configured with same VNI, and between VTEPS with unique VNIs configured (local presence and remote presence of VNIs). How routes are advertised and installed when services in respective VNI needs to communicate with other services in same or different VNIs attached to different VTEPs. In my previous post I covered how we can use BGP to configure EVPN and VXLAN so they can create tunnels between them, how they find each other. I have not covered how VTEPS know which VNI they should be interested in or inform the other VTEPS which VNI they are interested in. It may not be a surprise, but this is what route type 3 will solve. 

 Enabling broadcast domains across the VTEPS in the fabric. 

VM mobility, arp garp



## Egress

EVPN Route type 2 is there for participating VTEPs to exchange layer 2 mac-address information for any VNI that is "stretched" over multiple VTEPS in a fabric. It also supports exchanging IP informatuon for routing needs, routing VNI to VNI either asymmetric or symmetric (a more efficient way in large scale environments) using L3VNI.

Route type 3 is a there to inform the VTEPS which VNI they are interested in.
